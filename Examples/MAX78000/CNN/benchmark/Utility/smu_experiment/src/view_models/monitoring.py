...
class MonitorViewModel(QObject):
    ...
    def update_current_state_text(self, QLabel: QLabel, state: str, cell_index:int) -> None:
    ...
    def __call__(self, data: CellDataRow | None) -> None:
        ...
        if (
            ...
            drain_current = data.drain_current
            self.update_latest_text.emit(drain_current)
        ...
        if data.state == SequentialState.cell_sweep:
            ...
            gate_voltage = data.gate_voltage
            drain_current = data.drain_current
            self.update_plot_signal.emit(data.cell_index, gate_voltage, drain_current)

        if data.state == SequentialState.idle:
            self.data.clear()

        if data.state == SequentialState.end:
            self.update_current_state.emit("Configuring for next experiment", data.cell_index + 1)
        else:
