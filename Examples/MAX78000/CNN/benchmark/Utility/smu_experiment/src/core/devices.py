...
class SourceMeter(AbstractSourceMeter):
    ...
    def __init__(
        ...
        current_level: str = "10.0e-3",
        voltage_range: str = "100.0",
        voltage_set: str = "5.0",
        *args,
        **kwargs,
    ):
    ...
    def measure_voltage(self):
        ...
        if self._prev_state != SourceMeter.StateEnum.readVoltage:
            ...
            # Add this to the previous state, to prevent the reenter of this?
        data = self._resource.query_binary_values(
            ":meas:volt?", datatype="f", container=np.ndarray, is_big_endian=True
        )
        return data
...
class TracingSourceMeter(SourceMeter):
    def __init__(
        self,
        name: Optional[str] = None,
    ...
    def configure_trace(self, trace:str, modality:str) -> None:
        """Sets the trace mode for the SMU"""

        # Set to nev to allow for formatting of the trace
    ...
    def clear_trace(self, trace:str) -> None:
        ...
        # set to nev to clear trace
        self._resource.write(f":{trace}:feed:cont nev")
        self._resource.write(f":{trace}:cle")

    def get_free_trace(self, trace:str) -> Tuple[int, int]:
        """Get free memory from trace"""

        # Returns 2 variables, the first being the available data, the last being the total amount
        data = self._resource.query(f":{trace}:free?")

        return data
    
    def start_current(self, trace: str = "trac"):

        # Allow writing
        self._resource.write(f":{trace}:feed:cont next")

        # Set trigger to idle
        self._resource.write(f":abor:acq")
        self._resource.write(f":trig:acq:del 0")
    
    def set_trigger(self, rate: str, samples: str, trace: str = "trac"):

        # Set trigger intervals. Max sample rate is 2e-5, set by "min" as rate.
    ...
    # def measure_current(self, trace:str="trac") -> np.ndarray:
        
    #     # self._resource.query_binary_values(f":{trace}:data?")
    #     if not self.traced.get(trace, False):
    ...
    #     data = self._resource.query_binary_values(f":{trace}:data?")
    #     return data
    
    # def measure_voltage(self, trace:str="trac2"):
    #     self._resource.write(f":{trace}:feed:cont nev")
    #     self._resource.write(f":{trace}:feed sens")
    #     if not self.traced.get(trace, False):
