...
const command_t user_commands[] = {
    ...
    { "ri", "ri <binary file name>", "Run Inference on a binary file on SD Card", handle_inference }    // added to run individual inferences
    // ,{ "rai", "rai", "Run Inferences on all binary files in /bin on SD Card", handle_rai }
};

const unsigned int num_user_commands = sizeof(user_commands) / sizeof(command_t);

// int handle_inference(int argc, char *argv[]){
//     
// }
...
{
    if(argc != 2) {
        printf("Incorrect usage, please provide the file name. \n");
        return E_INVALID;
    }
    // printf(argv[1]);
    // Processes the voice vector from the desired file immediately
    return readAllFilesInBinDir(argv[1]);
}
