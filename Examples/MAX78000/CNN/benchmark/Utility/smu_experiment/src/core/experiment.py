...
class ExternalReferenceStrategy(AbstractConnectionStrategy):
    ...
    def connect_cell(self, cell_name: str) -> None:
        ...
        self.mux.connect("com8", "ch69")

    def disconnect_cell(self, cell_name: str) -> None:
        config = Config.get_instance()
        cell_channel = config.cell_channel_mapping[cell_name]
        self.mux.disconnect(cell_channel, self.vgs_channel)
        self.mux.disconnect("com8", "ch69")
...
@deprecated(version="0.1.0", reason="This sampling strategy is susceptible to freezing")
        ...
        try:
            ...
            with ExitStack() as stack:
                ...
                match self.config.mux_topology:
                    case "2524/1-Wire 128x1 Mux":
                        pass#connection_strategy = ExternalReferenceStrategy(multiplexer)
                    case "2524/1-Wire Dual 64x1 Mux":
                        connection_strategy = ExternalReferenceStrategy(multiplexer)
                    case "2524/1-Wire Quad 32x1 Mux":
                        connection_strategy = OnChipStrategy(multiplexer)
                    case _:
                        raise Exception("Invalid Mux Topology")
...
def play_audio(buffer, event):
    chunk=16000
    idx=0
    while not event.is_set():
        sd.play(buffer[idx*chunk:(idx+1)*chunk], blocking=True)
        idx += 1
        if (idx+1) * chunk > len(buffer):
            break 
...
class SequentialEFGETExperiment(AbstractExperiment):
    ...
    run = (
        _states.pause.from_(
            _states.cell_sweep, _states.stability_sweep, cond="pause_resume_triggered"
        )
    ...
    # Normal Flow
    run |= (
        _states.idle.to(_states.wait_start)
        ...
        | _states.wait_record.to(_states.cell_sweep, cond="user_event_triggered")
        ...
        | _states.verify_sweep.to(_states.wait_start)
    )
    ...
    def __init__(
        ...
        self.crab_rave_buffer, _ = sf.read("C:\\Users\\lmacosta\\Documents\\Python\\EGFET\\egfet-experiment-controls\\src\\assets\\crab_rave.mp3")
    ...
    def is_vgs_stable(self):
        ...
        if recorded_complete_sweep and self._prev_stability_data:
            close = np.allclose(
                self._prev_stability_data,
                self._stability_data,
                rtol=self.config.stability_threshold,
                atol=1e-10,
            )
    ...
    def on_enter_wait_start(self):
        ...
        self.crab_rave = Process(target=play_audio, args=(self.crab_rave_buffer, self.crab_rave_shutdown))
    ...
    def on_enter_stability_sweep(self):
        ...
        drain_current = self.vds_meter.measure_current()
        data = CellDataRow(
            0,
    ...
    def on_enter_wait_record(self):
        ...
        self.logger.debug("Waiting for User Start Record")
        data = CellDataRow(
            0,
...
class ReversedSequentialEGFETExperiment(SequentialEFGETExperiment):
    ...
    def on_enter_stability_sweep(self):
        ...
        self.vds_meter.set_voltage(gate_voltage)
        drain_current = self.vgs_meter.measure_current()
        data = CellDataRow(
            0,
            SequentialState.stability_sweep,
            self.vgs_index,
