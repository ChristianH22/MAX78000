...
class AddCellDialogBuilder:
    ...
    def build(
        ...
        channels_including_references = parent_vm.get_channel_connection_display_data()
        channels_including_references = [
            ch
            for (ch, role) in channels_including_references
            if role != "Undefined" or "Reference" in role
        ]
        ...
        match top_name:
            ...
            case "External Reference":
                add_cell_ui_file = files("views.qt.ui_files").joinpath(
                    "external_reference_add_cell_dialog.ui"
                )
            case _:
...
class MainViewBuilder:
    ...
    def build(
        ...
        view.actionStableMean.triggered.connect(content_vm.set_stable_mean_sampling_mode)
        view.actionMean.triggered.connect(content_vm.set_mean_sampling_mode)
        
        update_sweep_mode = partial(content_vm.update_sweep_mode_ui, view.sweep_controls_label, view.fixed_source_label)
